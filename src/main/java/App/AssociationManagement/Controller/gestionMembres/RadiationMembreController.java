...
public class RadiationMembreController {
    ...
    protected void onButtonValiderClick(ActionEvent event) {
        if (selectedMember == null) {
            Message.showAlert("Erreur", "Veuillez sélectionner un membre à radier.");
            return;
        }

        boolean cotisationPayee = selectedMember.get("cotisationPayee").asBoolean();
        if (cotisationPayee) {
            Message.showAlert("Impossible", "Ce membre ne peut pas être radié car il a payé sa cotisation.");
            return;
        }

        Alert confirmationAlert = new Alert(Alert.AlertType.CONFIRMATION, "Voulez-vous vraiment radier ce membre ?", ButtonType.YES, ButtonType.NO);
    ...
    private void radierMembre() {
        if (selectedMember == null) {
            Message.showAlert("Erreur", "Veuillez sélectionner un membre à radier.");
            return;
        }

        // Ajouter l'attribut de radiation au membre
        ...
        archivedMember.put("radiation", true);
        ...
        // Ajouter le membre radié au fichier d'archives
        ...
        try {
            ...
            Message.showInformation("Succès", "Le membre a bien été radié et archivé.");
        } catch (IOException e) {
